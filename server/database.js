...
export const medicationQueries = {
  ...
  // UPDATED: Now accepts archive reason and comments
  updateStatus: (medicationId, status, archiveReason, archiveComments) => {
    const stmt = db.prepare(`
      UPDATE medications
      SET status = ?, 
          ...
          archive_comments = CASE WHEN ? = 'archived' THEN ? ELSE archive_comments END
      WHERE id = ?
    `);
    const result = stmt.run(status, status, status, archiveReason, status, archiveComments, medicationId);
    return result.changes;
  },

  // NEW: Update archive info for already archived medications
  updateArchiveInfo: (medicationId, archiveReason, archiveComments) => {
    const stmt = db.prepare(`
      UPDATE medications
      SET archive_reason = ?,
          archive_comments = ?
      WHERE id = ?
    `);
    const result = stmt.run(archiveReason, archiveComments, medicationId);
    return result.changes;
  },
